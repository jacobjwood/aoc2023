...
fn main() {
    ...
    let part1_one_liner: i32 = contents
        .lines()
        .map(|l| l.chars().filter(|c| c.is_numeric()).collect::<String>())
        .map(|s| {
            (s.chars().nth(0).unwrap().to_string() + &s.chars().rev().nth(0).unwrap().to_string())
                .parse::<i32>()
                .unwrap()
        })
        .sum();
    ...
    let part2: i32 = contents
        .lines()
        .map(|l| part2_parse_line(l, &parse_map))
        .sum();
...
fn part2_parse_line(line: &str, parse_map: &HashMap<String, i32>) -> i32 {
    ...
    (left_amount.to_string() + &right_amount.to_string())
        .parse::<i32>()
        .unwrap()
}
